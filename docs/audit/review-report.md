...
剩余 28 条（P2×2 + P3×26，含部分修复的残余子项）均有明确处置理由，建议按以下三档排期：

### 第一档：低成本快修（可随任意一批提交顺手处理）
1. **t3-F22 之类型收窄**：`App.vue:150` `currentTheme` 补 `'verdant' | 'dracula'`（一行改动，验证报告已点名；当前仅靠 Vite 不做类型检查而未报错）。
2. **t1-P3-8 之文档措辞**：`docs/wasm_core.md:5`（及中文版对应句）「behavior-identical」收敛为「语义等价，行内格式保真度有差异」（代码分歧已收敛，纯文档改动）。
3. **t1-P3-9 之文档注记**：在 `docs/wasm_core.md` i32 约束条目旁补「i64 字段须保持 <2^53」。
4. **t1-P3-1**：Rust 端标题级别 `level.min(6)` 收口并对齐 JS 回退 `/^h[1-6]$/`（改动极小，消除唯一已知双实现分歧之一）。
5. **t2-F4 之权限复制**：rename 前 `set_permissions` 复制原 mode（避免 0600 文件保存后放宽为 0644 的隐私回退）。
6. **t4-F-14 / t4-F-16**：SECURITY 双语补具体联系渠道（GitHub Private vulnerability reporting）；rendering_review 的 http 链接改 https。

### 第二档：专项（需要设计与测试投入，建议纳入下一迭代）
1. **t1-P2-3（P2，最高优先）**：Rust 侧 `children_to_markdown`/`collect_text` 显式栈迭代化 + `DomNode` 反序列化入口限深。当前靠「WASM trap → JS 回退（已限深 512）」优雅失败，但 wasm32 栈阈值低，恶意/病态页面可稳定触发功能失效。
2. **t1-P3-3 余项**：文本节点 `*`/`_`/`#`/`[` 与链接 `]`/`)`、URL 括号的最小转义——JS/Rust 双侧同步落地（双侧目前是「一致缺口」，单独修任一侧会制造新分歧，须成对修）。
3. **t3-F16 i18n 接入**：以 `shared/i18n.ts` 为入口接入 `chrome.i18n`，UI 硬编码中文文案迁移至 messages.json（8 语言包基建已在）。
4. **t2-F2 之宿主白名单**：若计划面向企业/多用户场景分发，提前做 `allowed_paths` 前缀/后缀白名单；纯个人场景可继续沿用 SW 绑定方案。
5. **t2-F9 之 SW 超时 disconnect** + 超大文档超时放大策略。
6. **t3-F17**：文件访问权限未开启时的用户引导提示与空目录/失败状态区分展示。

### 第三档：结构性 / 发布前决策
1. **t3-F7（P2，唯一不修的 P2）**：内容脚本 5.2MB JS + 1.5MB CSS 的按需加载/分包，以及 `document_start → document_idle` 时序评估——涉及首屏体验权衡（原文闪烁），需要产品决策与专项性能测试。
2. **t3-F23**：构建产物显式化（内容脚本 CSS 文件名、manifest 引用文件存在性校验、sourcemap 策略）——Vite 升级前的防御性工程。
3. **t3-F18/F19/F20/F21**：目录请求错误路径清理、doc-stats 分语言统计、空 slug 回退前缀、接管判定放宽——可合并为一个「目录与接管健壮性」批处理。
4. **t1-P3-4/5/6/7 与 t2-F5/6/7/8/10/11**：均为低风险改进建议，随对应模块的下一次迭代顺手处理即可（t2 各项在分发形态变化——如 Windows 支持、商店打包——时再评估）。
